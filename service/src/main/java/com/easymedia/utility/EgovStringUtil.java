...
public class EgovStringUtil {
	...
	/**
	 * 응용어플리케이션에서 고유값을 사용하기 위해 시스템에서 17자리의 TIMESTAMP와 난수를 조합
	 *
	 * @param
	 * @return Timestamp 값
	 * @see
	 */
	public static String getRandomTimeStampChar(String time, String[] alpha, Random random)
	{
		String rtnStr = "";
		String ranStr = "";
		for(int q = 0 ; q < 12 ; q++)
		{
			rtnStr = rtnStr + alpha[random.nextInt(alpha.length-1)];
		}
		ranStr = rtnStr + time;
		String[] ranStrArray = ranStr.split("");
		for (int i = ranStrArray.length - 1; i > 0; i--)
		{
		  int index = random.nextInt(i + 1);
		  String a = ranStrArray[index];
		  ranStrArray[index] = ranStrArray[i];
		  ranStrArray[i] = a;
		}
		return StringUtils.join(ranStrArray, "");
	}
	...
	public static String getUrlFolder(String str)
  	{
  		...
  		if (str.indexOf(".do") > -1)
  		{
  			...
  			for (int i = 0, size = arrFolder.length - 1; i < size; i++)
  			{
  				val = arrFolder[i];
	...
	/**
	 * <p>YYYY-MM-DD의 날짜를 받아서 분기를 반환</p>
	 *
	 * <pre>
	 *   quarterYear("2010-09-01")
	 * </pre>
	 *
	 * @param yyyyMMdd
	 * @return " - "가 추가된 입력문자열
	 */
	public static int quarterYear(String yyyyMMdd)
	{
	...
	/**
	 * <p>YYYY-MM-DD의 날짜를 받아서 분기, 반기, 년의 해당 년월을 반환</p>
	 *
	 * <pre>
	 *   quarterYear("2010-09-01")
	 * </pre>
	 ...
	 * @return " - "가 추가된 입력문자열
	 */
	public static String quarterYearMonth(String yyyyMMdd, String quarterType)
	{
		String date = yyyyMMdd.replace("-", "");
		String yyyy = date.substring(0, 4);
		int mm   = Integer.parseInt( date.substring(4, 6) );
		String tmpMM = "";
		if("2".equals(quarterType))
		{
			//반기
			if(mm >= 7)
			{
